...
class RedisStorageMixin(object):
    ...
    def __init__(self, request, *args, **kwargs):
        self.client = redis.StrictRedis(host=STORED_MESSAGES_REDIS.get('host'),
    ...
    def _get(self, *args, **kwargs):
        ...
        if self.user.is_authenticated():
            inbox_messages = self.client.get('user:%d:notifications' % self.user.pk)
        else:
            inbox_messages = []
    ...
    def _store(self, messages, response, *args, **kwargs):
        ...
        if self.user.is_authenticated():
            if not messages:
                ...
                for m in messages:
                    self.client.sadd('user:%d:notifications' % self.user.pk, m)
...
class RedisPersistentStorage(RedisStorageMixin, FallbackStorage):
    """
